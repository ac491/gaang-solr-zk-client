...
function getSolrInstance(solrBundles, solrCollection, callback) {
    let instances = []
    solrBundles.forEach(bundle => {
        if(bundle.collections.indexOf(solrCollection) > -1) {
            instances.push(bundle.instance);
        }
    })
    if (!instances.length) {
...
function createSolrCollectionGETRequestTasks(zkInfo, options) {
    return zkInfo.liveNodes.map(function (liveNode) {
        ...
        return function (callback) {
            got(protocolLiveNode + options.solrCollectionsGetEndPoint, options.rest).then(response => {
